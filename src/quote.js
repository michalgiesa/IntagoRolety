...
export function configurationId(state){
 const data=JSON.stringify([state.orientation,state.orientation==='horizontal'?(state.horizontalDirection||'right'):null,state.mounting,state.furniture,state.dimensions,state.selectedProduct,state.finish,DATA_VERSION]);
...
export function configurationText(s){
 ...
 ...(s.orientation==='horizontal'?['Strona otwierania: '+(s.horizontalDirection==='left'?'w lewo':'w prawo')]:[]),
